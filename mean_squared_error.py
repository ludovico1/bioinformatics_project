...
def calc_error_cresc(refhisto, histo, tool):
    ...
    while i < len(ref_tuples) and j < len(data_tuples):
        if int(ref_tuples[i][0]) == int(data_tuples[j][0]):
            summ+=((ref_tuples[i][1] - data_tuples[j][1]) ** 2)
            count+=1
            tmp = m.sqrt(summ/count)
    ...
    error_1_file = histo + '_SD_crescente.HISTO'
...
def calc_error_decr(refhisto, histo, tool):
    ...
    error_1_file = histo + '_SD_decrescente.HISTO'
    with open(error_1_file, 'w') as hf:
        for j in error_1:
            hf.write("{} {}\n".format(j[0], j[1]))
        hf.close()
...
def sd_error(refpah,crespath,decrpath,typecres,typedecr,graphdir,datasetType):
    if (typecres == "SAKEIMA"):
        colorcres="orange"
    ...
    title = 'SD error ' + typecres + ' vs ' + typedecr + ' ' + datasetType
    ...
    ax.legend([typecres + ' SD error crescente', typedecr + ' SD error decrescente'])
    ax.set(title=title, xlabel='Abundance', ylabel='SD Error')
