...
parser.add_argument("-w", help="path to the workspace")
parser.add_argument("-bin", help="path to bin folder sakeima")
...
def get_result(pattern , path ,  verbose=1):
    ...
    for line in fin:
        if pattern in line:
            ...
            if verbose == 1:
                print(line)
            print(line[len(pattern):])
            tmp=line[len(pattern):]
...
def get_total_positions(dataset):
    ...
    jellyfish_path = os.path.join(args.bin,"/bin/jellyfish")
    work_dir_path = os.path.join(args.w, "work_dir/")
    ...
    if not args.ell:
        if not args.epsilon:
            if not args.theta:
                ...
                print("TROVATO LOWEST THETA:  "+str(find_lowest_theta(sample_size)))
                args.epsilon = default_value_epsilon(args.theta , args.dbtot)
                args.ell = default_value_ell(args.theta)
            else:
                args.epsilon = default_value_epsilon(args.theta , args.dbtot)
                ...
                if args.ell < 0:
                    print(
                        "Impossible to compute the approximation. use a lower value for theta or a larger sample size.")
                    exit()
        else:
            args.ell = find_lowest_ell(args.epsilon , sample_size)
            if args.ell < 0:
...
def run_sakeima():
    ...
    print("bin directory: " + str(args.bin) + "...")
    jellyfish_path = str(args.bin) +"bin/jellyfish"
    print("jellyfish path: " + str(jellyfish_path) + "...")
    work_dir_path = str(args.w) + "/work_dir/"
    if not os.path.exists(work_dir_path):
        os.system("mkdir "+str(work_dir_path))
    temp_file_path = work_dir_path+"out_temp.txt"
    # agg. 04062021 aggiungo file esterno per passaggio sample size
    ssfile = work_dir_path + "samplesize.txt"
    ## end agg
    if not args.output:
        path_counts = work_dir_path+"counts_SAKEIMA_"+str(args.k)+".txt"
    else:
    ...
    print(">>> Samplesize INIZIALE "+str(sample_size))
    m_old = m
    m = round(sample_size / args.ell)
    ...
    #####agg 0406
    ### varie prove
    #ratio_sample=sample_size / sample_size_old
    #ratio_sample= sample_size / args.dbtot
    #ratio_sample = sample_size / sample_size_old
    #### Valore corretto della correzione e' lambda
    ### infatti è il numero di kmers presi rispetto il totale

    ratio_sample =args.lambd
    with open(ssfile, "w") as pf:
        print("Sakeima2 scrittura sampling size")
        pf.write(str(ratio_sample)+ " \n")


        print(" Sakeima2 scrittura sampling size, sampling size:" + str(ratio_sample))
        #pf.write(" Sakeima2 scrittura sampling size, sampling size:" + str(ratio_sample)+ " \n")
        print(" dimensione originale:" + str(sample_size_old))
        #pf.write(" dimensione originale:" + str(sample_size_old)+ " \n")
        print(" dimensione sample : "+str(sample_size))
        pf.write(str(sample_size)+ " \n")
        #pf.close()
